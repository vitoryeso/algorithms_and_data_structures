...
class MatMulBenchmark:
 ...
 def generate_test_code(self, algorithm: str, size: int, min_val: int = -10, max_val: int = 10, seed: int = 42) -> str:
     ...
     Args:
         ...
         min_val: Valor mínimo para elementos da matriz
         max_val: Valor máximo para elementos da matriz
...
"""
 ...
 def save_results(self, results: List[Tuple[int, float, float, float, float]], filename: str):
     """
     ...
     with open(filename, 'w', newline='') as csvfile:
         ...
         writer.writerow(['size', 'naive_time', 'strassen_time', 'blocked_time', 'blocked_opt_time'])
         for size, naive_time, strassen_time, blocked_time, blocked_opt_time in results:
             writer.writerow([size, naive_time, strassen_time, blocked_time, blocked_opt_time])
     print(f"Resultados salvos em {filename}")

 def plot_results(self, results: List[Tuple[int, float, float, float, float]], filename: str = None, theory_curves: List[str] = None):
     """
     ...
     blocked_times = [r[3] for r in results]
     blocked_opt_times = [r[4] for r in results]

     plt.figure(figsize=(14, 10))

     # Plota os tempos experimentais
     plt.plot(sizes, naive_times, 'o-', label='Naive (O(n³))', linewidth=2, markersize=6)
     plt.plot(sizes, strassen_times, 's-', label='Strassen (O(n^2.81))', linewidth=2, markersize=6)
     plt.plot(sizes, blocked_times, '^-', label='Blocked (64)', linewidth=2, markersize=6)
     plt.plot(sizes, blocked_opt_times, 'D-', label='Blocked Opt (1000)', linewidth=2, markersize=6)     # Adiciona curvas teóricas de complexidade (baseline comum)
     if sizes:
     ...
     plt.title('Comparação: Naive vs Strassen vs Blocked Algorithms\nCache-Optimized Matrix Multiplication', fontsize=16)
     # Legenda e grid
     plt.legend(fontsize=12)
     plt.grid(True, alpha=0.3)

     # Anotações com valores
     for i, (size, naive, strassen, blocked, blocked_opt) in enumerate(results):
         if naive != float('inf'):
         ...
         if blocked != float('inf'):
             plt.annotate(f"{blocked:.3f}", (size, blocked),
                         textcoords="offset points", xytext=(0,-5), ha='center', fontsize=8)
         if blocked_opt != float('inf'):
             plt.annotate(f"{blocked_opt:.3f}", (size, blocked_opt),
                         textcoords="offset points", xytext=(0,-15), ha='center', fontsize=8)
     plt.tight_layout()
 ...
 def load_results_from_csv(self, filename: str) -> List[Tuple[int, float, float, float, float]]:
     """Carrega resultados (size, naive_time, strassen_time, blocked_time, blocked_opt_time) de um CSV previamente salvo."""
     ...
     with open(filename, 'r') as f:
         ...
         for row in reader:
             try:
                 ...
                 blocked_time = float(row[3]) if len(row) > 3 else float('inf')
                 blocked_opt_time = float(row[4]) if len(row) > 4 else float('inf')
                 rows.append((size, naive_time, strassen_time, blocked_time, blocked_opt_time))
             except Exception:
                 continue
     return rows
...
def main():
 ...
 parser = argparse.ArgumentParser(
     ...
     epilog="""
Exemplos de uso:
%(prog)s --sizes 2 4 8 16 32 64 128 256 512 1024 --runs 5 --min-val 0 --max-val 1
%(prog)s --max-size 2048 --runs 3 --min-val -1000 --max-val 1000
%(prog)s --sizes 64 128 256 512 1024 2048 4096 8192 --output meus_resultados.csv
 ...
 # Novos argumentos para min_val e max_val
 parser.add_argument('--min-val', type=int, default=-10,
                    help='Valor mínimo para elementos das matrizes (padrão: -10)')
 parser.add_argument('--max-val', type=int, default=10,
                    help='Valor máximo para elementos das matrizes (padrão: 10)')
 ...
 print(f"  Valores das matrizes: [{args.min_val}, {args.max_val}]")
 ...
 # Executa benchmark com min_val e max_val configurados
 results = benchmark.run_benchmark(args.sizes, args.runs, args.min_val, args.max_val)
